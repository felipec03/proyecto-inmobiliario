# Product Backlog — MiLocal (MVP)

> Documento único de épicas e historias de usuario del MVP.
> Alineado con `docs/MiLocal_Enfoque_MVP_Paso_a_Paso.docx`, `docs/DEFINICION_PROBLEMA.md`,
> `docs/charter/PROJECT_CHARTER.tex`, `docs/matchmaking/*`, `docs/database/schema.dbml` y
> `docs/api/openapi.yaml`.

## 0. Alcance y trazabilidad

Componentes del MVP (`.docx`, sección 2) y su épica correspondiente:

| Componente MVP | Épica |
|----------------|-------|
| Infraestructura y arquitectura base | Épica 1 |
| Perfil emprendedor / propietario + Trust Level | Épica 2 |
| Assessment y preferencias | Épica 3 |
| Publicación de local y catálogo | Épica 4 |
| Matching V1 | Épica 5 |
| Portal público | Épica 6 |
| Backoffice MiLocal + interés/contacto + métricas | Épica 7 |

**Notas de alcance (decisiones cerradas):**

- **Matching V1:** se implementa el motor **Weighted Feature Distance** (Épica 5) como versión
  inicial automatizada, complementado con **reglas simples y apoyo manual** del equipo (HU17).
  Esto respeta el principio del `.docx` de "no automatizar el 100% del scoring" durante la
  validación temprana.
- **Trust Level:** la verificación de documentos la realiza un **administrador interno**
  (HU08 y HU22), no el propio usuario.
- **Tendencias de mercado:** se resuelven con **datos estáticos curados** en la tabla
  `static_trends`, sin dependencia de LLM (HU23 y `docs/trends/ALTERNATIVAS_SIN_IA.md`).
- **Fuera de alcance MVP:** chatbot IA, Visual Analyzer, integración con buró de crédito,
  KYC biométrico y scoring financiero sobre carpeta tributaria. Ver
  `docs/FUERA_DE_ALCANCE_LLM.md` y `docs/DEFINICION_PROBLEMA.md`.

**Formato de IDs:** `HUxx` (dos dígitos), `HU-xx` solo en referencias históricas.

---

# Épica 1: Infraestructura y Arquitectura Base

> Configurar los repositorios y el pipeline CI/CD utilizando Jenkins. Implementar la
> infraestructura inicial con Docker Compose. Esta infraestructura debe incluir los servicios
> para la base de datos PostgreSQL 16, el backend en Go 1.23 y el frontend. Diseñar el esquema
> de base de datos con las tablas requeridas para almacenar usuarios, documentos, propiedades
> y variables de preferencias.

## HU01: Configuración de Infraestructura y Contenedores Locales
...
- **Tareas:**
    ...
    - Configurar el servicio de base de datos (`milocal-db`) con la imagen `postgres:16-alpine`, mapeando el puerto 5434 del host al 5432 del contenedor.
    - Configurar el servicio del backend (`milocal-api`) con un binario de Go 1.23, mapeando el puerto 8082 del host al 8080 del contenedor.
...
- **Tareas:**
    ...
    - Crear la tabla `properties` con todas las columnas del modelo (27 en total: datos generales, 7 especificaciones técnicas del matcher, metadata, `owner_id`, `available` y timestamps), incluyendo el campo booleano `available` para registrar disponibilidad/vacancia del local.
    - Crear la tabla `user_preferences` definiendo campos estructurados como `rubro`, `max_budget`, `min_size`, `max_size` y `commune_id` (relación 1:1 con el usuario).
    - Crear las tablas transaccionales y de registro restantes: `matches` (con campo `status` para outcomes: `scored`, `contacted`, `closed_won`, `closed_lost`), `chat_history` y `assessments` (esta última estructurada con un campo tipo JSONB para almacenar los datos del onboarding).
    - Crear la tabla `leads` que registre guardados, postulaciones y solicitudes de contacto entre emprendedores y propiedades (relación con `users` y `properties`).
...
- **Tareas:**
    - Inicializar y configurar el repositorio central del proyecto en GitHub (`https://github.com/felipec03/proyecto-inmobiliario`).
    - Crear la estructura de directorios del backend en Go, definiendo específicamente las carpetas `cmd/server`, `internal/handler`, `internal/service` y su submódulo `internal/service/matcher`, `internal/repository`, `internal/middleware` (middlewares), `internal/auth` y `internal/model`.
...
> Desarrollar los flujos de registro e inicio de sesión integrando JWT para la autenticación y
> bcrypt para el manejo seguro de contraseñas. Construir los perfiles separados para
> emprendedores y propietarios. Implementar el sistema de Trust Level permitiendo a los usuarios
> subir sus documentos de verificación. Los documentos incluyen identidad, comprobantes de
> ingresos, antecedentes legales y títulos de propiedad.
...
- **Tareas:**
    ...
    - Crear un middleware de autenticación bajo el directorio `internal/auth/` que intercepte las peticiones y verifique la validez del JWT en todos los endpoints protegidos.
...
- **Tareas:**
    ...
    - Persistir el documento con estado inicial `pending` al momento de la subida (nunca `verified` directamente; solo el administrador puede marcarlo como verificado).

## HU08: Sistema de Trust Level (Verificación Interna por Administrador)
...
- **Criterios de Aceptación:** El administrador puede aprobar documentos y el sistema transiciona automáticamente el nivel de confianza del usuario evaluado.
- **Tareas:**
    - Desarrollar el endpoint `POST /users/{id}/documents/{docId}/verify` protegido con JWT y restringido a administradores, destinado a asentar la revisión administrativa de los comprobantes.
    - Programar la lógica de base de datos para que los usuarios inicien con un Trust Level de valor "0" (no verificado) y, al tener sus 3 documentos obligatorios verificados exitosamente (identidad, ingresos y legal), asciendan a Trust Level "1". El documento de propiedad queda como opcional.
...
> Crear un formulario o assessment breve para captar el rubro, presupuesto, tamaño y
> requerimientos técnicos requeridos por el emprendedor. Desarrollar el assessment del
> propietario para recopilar información sobre el inmueble, la renta solicitada y el perfil de
> arrendatario deseado. Asegurar que todas las respuestas de este proceso se almacenen de forma
...
- **Tareas:**
    ...
    - Incorporar al assessment las variables de segmentación del `.docx`: **etapa del negocio** (idea/en marcha/consolidado), **nivel de formalización** (informal/persona natural/empresa) y **características críticas** del local buscado.
...
- **Tareas:**
    ...
    - Incorporar las variables del `.docx` para el propietario/local: **infraestructura disponible** y **disponibilidad/vacancia** del inmueble.
...
- **Tareas:**
    ...
    - Mapear y persistir las respuestas consolidadas en la tabla `user_preferences`, registrando obligatoriamente los campos `rubro`, `max_budget`, `min_size`, `max_size` y `commune_id` (relación 1:1 con la tabla users).
...
> Construir el flujo para publicar inmuebles y capturar sus especificaciones. Integrar la
> captura obligatoria de las dimensiones técnicas del inmueble, tales como capacidad eléctrica,
> conexiones de agua y gas, trampa de grasa y métricas de flujo peatonal. Desarrollar los
> endpoints (CRUD) para que los propietarios administren sus propiedades y suban las imágenes
> de los locales.
...
- **Tareas:**
    ...
    - Construir la interfaz de publicación en el frontend garantizando la captura de las dimensiones técnicas clave: capacidad eléctrica, conexión de gas, conexión de agua, trampa de grasa, frente comercial, flujo peatonal, disponibilidad de precio negociable y disponibilidad/vacancia del local.
    - Persistir la información del inmueble en la tabla `properties`, garantizando el soporte para las 27 columnas del modelo (7 de ellas corresponden a las especificaciones técnicas del matcher).
...
- **Tareas:**
    - Desarrollar el endpoint `POST /upload/property-image/{id}` protegido con JWT para recibir los archivos.
...
> Implementar la versión inicial del motor de matchmaking utilizando el algoritmo de Weighted
> Feature Distance. Configurar los 6 perfiles de rubro base y aplicar las fórmulas de puntaje
> para evaluar el ajuste de las dimensiones técnicas y las preferencias. Implementar reglas
> duras en el cálculo, aplicando un límite (cap) del 29% al puntaje si el negocio del usuario
> no está dentro de los usos permitidos del local. Complementar este motor automatizado con
> capacidades de revisión humana y reglas simples para apoyar la validación del modelo en la
> fase temprana.
...
- **Tareas:**
    - Implementar la lógica central del algoritmo Weighted Feature Distance en el directorio `internal/service/matcher/` del backend.
    - Desarrollar el endpoint `POST /match` protegido con JWT para ejecutar la evaluación de compatibilidad entre un usuario y un inmueble específico a demanda.
...
- **Tareas:**
    - Configurar la persistencia de los resultados de cada evaluación en la tabla `matches` de PostgreSQL, incluyendo el campo `status` (`scored`, `contacted`, `closed_won`, `closed_lost`) para mantener un historial auditable del motor.
...
> Desarrollar una interfaz con diseño responsive adaptable a plataformas móviles y de
> escritorio. Crear el buscador y el catálogo público para facilitar el descubrimiento de
> locales comerciales. Implementar las vistas de detalle del inmueble que muestren al usuario
> el desglose explicable de su puntaje de compatibilidad para cada dimensión evaluada.
...
> Construir una vista interna de backoffice para que el equipo pueda administrar usuarios,
> inmuebles, variables y gestionar los leads. Integrar funcionalidades que permitan a los
> usuarios guardar locales, postular, y enviar o recibir solicitudes de contacto. Registrar
> métricas operativas clave para analizar qué variables determinan si un usuario acepta o
> descarta un match.
...
- **Tareas:**
    ...
    - Desarrollar la lógica y los endpoints en el backend (`POST /properties/{propertyId}/contact`) para registrar y gestionar estas solicitudes de contacto o respuestas generadas en la plataforma.
    - Persistir cada interacción en la tabla `leads` (usuario, propiedad, tipo de interacción y estado) para permitir su seguimiento y trazabilidad.
...
- **Tareas:**
    ...
    - Desarrollar los endpoints administrativos protegidos por rol admin: listado y edición de usuarios, inmuebles, variables capturadas, leads y estado individual de cada match.
    - Integrar las capacidades para que el administrador de la plataforma pueda gestionar el catálogo y realizar la revisión manual de los documentos (`POST /users/{id}/documents/{docId}/verify`) para ajustar el Trust Level de los usuarios.
...
- **Tareas:**
    ...
    - Configurar el registro analítico para identificar y registrar en la base de datos qué variables explican la aceptación o el descarte de un match (actualizando el `status` de `matches`).
    ...
    - Implementar el dashboard de tendencias de mercado para Chile alimentado por la tabla `static_trends` (datos estáticos curados de fuentes como Colliers/CBRE/JLL), según `docs/trends/ALTERNATIVAS_SIN_IA.md`, sin dependencia de LLM.
    - Crear el endpoint administrativo `PUT /admin/trends/{city}` para que el equipo actualice los datos de mercado de forma manual y trazable.
...
| Prioridad | Historias |
|:---|:---|
| **Must Have** | HU01–HU05, HU07, HU09, HU11–HU16, HU19, HU20 |
| **Should Have** | HU06, HU08, HU10, HU17, HU18, HU21, HU22 |
| **Could Have** | HU23 |
| **Won't Have (MVP)** | US.02, US.03, US.04, Chatbot IA, Visual Analyzer |

---

## Historias fuera de alcance MVP (backlog futuro)

Estas historias provienen del backlog histórico y quedan **explícitamente fuera del MVP**
según `docs/DEFINICION_PROBLEMA.md` y `docs/FUERA_DE_ALCANCE_LLM.md`:

| ID | Historia | Motivo |
|:---|:---|:---|
| US.02 | Carga de carpeta tributaria (SII) con cifrado AES-256 | Fase posterior; requiere integración financiera |
| US.03 | Trust Score ponderado con buró de crédito (Dicom/Equifax) | Fuera de alcance; el Trust Level actual es binario (0/1) |
| US.04 | Validación de identidad KYC biométrica | Fuera de alcance; sustituida por revisión documental del administrador |
| — | Chatbot Consultor IA | ON HOLD, pendiente proveedor LLM |
| — | Visual Analyzer | ON HOLD, requiere modelo multimodal |
| — | Pasarela de pagos, firma electrónica, app nativa, multi-país | Fase posterior |

Los requisitos de **US.01 (Perfilamiento de rubro)** quedan cubiertos por HU09 y HU15;
**US.05 (Dashboard de postulante / nivel de arrendatibilidad)** por el indicador de Trust
Level de HU08.
...
- Todo endpoint nuevo referenciado en `docs/api/openapi.yaml`.
- Toda tabla o columna nueva reflejada en `docs/database/schema.dbml`.

---

_Última actualización: Septiembre 2026 — Backlog consolidado y alineado con el Enfoque MVP, el charter y el esquema de base de datos._
